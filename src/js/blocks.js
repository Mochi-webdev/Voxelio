...
Blockly.Blocks['create_shape'] = {
    init: function () {
        ...
        // Change from FieldTextInput to ValueInput
        ...
        this.appendDummyInput()
            .appendField("in:")
            .appendField(new Blockly.FieldTextInput("Szene"), "PARENT");
...
GC.forBlock['create_shape'] = wrap((b, g) => {
    const type = b.getFieldValue('TYPE');
    const parent = b.getFieldValue('PARENT');
    const color = g.valueToCode(b, 'COL', ORDER_ATOMIC) || "'#ffffff'";

    // Get the name from the input. 
    // If empty, it creates a random ID like "obj_12345"
    let nameCode = g.valueToCode(b, 'NAME', ORDER_ATOMIC);
    if (!nameCode) {
        nameCode = `("obj_" + Math.floor(Math.random() * 999999))`;
    }

    return `App.spawn('${type}', ${color}, ${nameCode}, '${parent}');\n`;
